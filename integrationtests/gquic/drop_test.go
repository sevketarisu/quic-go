...
var _ = Describe("Drop tests", func() {
	...
	deterministicDropper := func(p, interval, dropInARow protocol.PacketNumber) bool {
		...
		Context(fmt.Sprintf("with QUIC version %d", version), func() {
			Context("during the crypto handshake", func() {
				for _, d := range directions {
					...
					It("establishes a connection when the first packet is lost", func() {
						startProxy(func(d quicproxy.Direction, p protocol.PacketNumber) bool {
							return p == 1 && d.Is(direction)
						}, version)
						downloadHello(version)
					})

					It("establishes a connection when the second packet is lost", func() {
						startProxy(func(d quicproxy.Direction, p protocol.PacketNumber) bool {
					...
					It("establishes a connection when 1/5 of the packets are lost", func() {
						startProxy(func(d quicproxy.Direction, p protocol.PacketNumber) bool {
